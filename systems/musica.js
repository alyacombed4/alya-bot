...
const queues = new Map(); // fila por servidor

async function getVideoUrl(query) {
  // Se já for um link do YouTube, usa direto
...
async function playSong(message, queue) {
  if (queue.songs.length === 0) {
    queue.playing = false;
    // Sai da call depois de 30s sem música
    setTimeout(() => {
      if (queue.songs.length === 0 && queue.connection) {
        queue.connection.destroy();
        queues.delete(message.guild.id);
...
module.exports = (client) => {
  // ... (mantenha todo o seu código atual aqui)
  ...
  client.on("messageCreate", async (message) => {
    ...
    if (command === "!p") {
      ...
      const query = args.join(" ");
      if (!query)
        return message.reply("❌ Informe o nome ou link da música.\nEx: `!p lofi hip hop`");

      const searching = await message.reply("🔍 Pesquisando...");

      try {
        const url = await getVideoUrl(query);
        if (!url) return searching.edit("❌ Nenhum resultado encontrado.");
    ...
    if (command === "!skip") {
      ...
      queue.player.stop(); // o evento Idle já chama o próximo
      message.reply("⏭️ Música pulada!");
    }
